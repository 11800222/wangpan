...
public class UserService {
	...
	public String isExist(HttpSession session, users user) {
		...
		if (list != null && list.size() > 0) {
			 ...
			 */
			System.out.println(user.getUsername()+"--------------was set into session");
	...
	public boolean countDown(String username) {
		users user = getUserByKey(username);
		if (user == null || user.getCount()<1)  //找不到用户和次数不够都返回false
			return false;
