...
public class FilesService {
	...
	public void CompleteFilesByID(Tasks t) throws Exception {
		String user = t.getUser();
		String url = t.getUrl();
		boolean flag = true;
		// 在离线下载文件夹下找到该文件;
		JSONObject item = ResponseUtil.getItemByid(
				GetResponse.openfile("437789382553092060", "0"), t.getId());
		// 修改根目录下文件的pid为user
		item.put("pid", user);
		// 宽搜同步该文件及其所有子文件
		...
		while (!queue.isEmpty()) {
			...
			JSONArray items = ResponseUtil.getSubItemsByid(GetResponse
					.openfile(item.getString("cid"), "0"));
	...
	public Files getFileByurl(String url) {
		List<Files> temp = filesdaoimpl.getCurrentSession()
				.createCriteria(Files.class).add(Restrictions.eq("url", url))
				.list();
		if (temp.size() == 0)
			return null;
		else
	...
	public List<Files> getFileByuser(String pid) {
		return filesdaoimpl.getCurrentSession().createCriteria(Files.class)
				.add(Restrictions.like("user", "%." + pid + ".%")).list();
	}
